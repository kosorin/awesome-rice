...
---@field actions_table? table<string, DesktopAction>
---@field command? string
---@field Type "Application"|"Link"|"Directory"
...
---
---[View specification](https://standards.freedesktop.org/desktop-entry-spec/latest/ar01s05.html)
---@type table<string, fun(key_file, key: string)>
local keys_getters
do
    local function get_string(key_file, key)
        return key_file:get_string("Desktop Entry", key)
    end

    local function get_strings(key_file, key)
        return key_file:get_string_list("Desktop Entry", key, nil)
    end

    local function get_localestring(key_file, key)
        return key_file:get_locale_string("Desktop Entry", key, nil)
    end

    local function get_localestrings(key_file, key)
        return key_file:get_locale_string_list("Desktop Entry", key, nil, nil)
    end

    local function get_boolean(key_file, key)
    ...
    keys_getters = {
        ...
        URL = get_string,
        PrefersNonDefaultGPU = get_boolean,
        SingleMainWindow = get_boolean,
    }

    -- Default to `get_string` if no getter was defined above.
    setmetatable(keys_getters, { __index = function() return get_string end })
end
...
function M.parse_desktop_file(path)
    ...
    local key_file = glib.KeyFile()
    if not key_file:load_from_file(desktop_file.path, glib.KeyFileFlags.NONE) then
        return nil
    end

    -- In case [Desktop Entry] was not found
    if not key_file:has_group("Desktop Entry") then
        return nil
    end

    for _, key in pairs(key_file:get_keys("Desktop Entry")) do
        local getter = keys_getters[key]
        desktop_file[key] = getter(key_file, key)
    end
    ...
    if desktop_file.Icon then
        desktop_file.icon_path = M.lookup_icon(desktop_file.Icon)
    end
        ...
        while true do
            ...
            for _, info in ipairs(list) do
                ...
                if file_type == "REGULAR" then
                    ...
                    if path then
                        local success, desktop_file = pcall(M.parse_desktop_file, path)
                        if not success then
